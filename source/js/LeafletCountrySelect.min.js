...
L.CountrySelect = L.Control.extend({
  ...
  onAdd() {
    ...
    const countries = (Array.isArray(this.options.include) && this.options.include.length > 0)
      ? this.options.include : this.options.countries;

    const countryKeys = Object.keys(countries).sort();
    /* eslint-disable */
    for (let i in countryKeys) {
  ...
  _onChange(e) {
    const selectedCountry = this.select.options[this.select.selectedIndex].value;
